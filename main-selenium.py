...
def _get_web_driver(url: str) -> Optional[object]:
    try:
        ...
        error(f"Failed to launch web driver: {e}")
...
def authenticate_with_selenium(
    ...
    print("Authorize access to your resources from this app")

    driver = _get_web_driver(code["auth_uri"])
    if driver is None:
        error("Failed to launch web driver")
        return None
    ...
    while True:
        try:
            ...
            for log in logs:
                ...
                if "message" in log and "params" in log["message"] and "documentURL" in log["message"]["params"]:
                    url = log["message"]["params"]["documentURL"]
                    if "urn:ietf:wg:oauth:2.0:oob" == url[:25]:
                        urn_url = url
                        break

            if urn_url is not None:
                break

        except selenium.common.exceptions.InvalidSessionIdException:
            break
        except Exception as e:
            error(f"Error while getting web driver logs: {e}")
            break
    ...
    if urn_url is None:
        error("Failed to get authorization code from web driver")
        return None

    auth_response_str = urn_url[26:]
...
def get_access_token(
    ...
    if auth_info is None:
        ...
        if redirect_uri == "urn:ietf:wg:oauth:2.0:oob":
            auth_info: dict = authenticate_with_selenium(app, scopes, redirect_uri)
